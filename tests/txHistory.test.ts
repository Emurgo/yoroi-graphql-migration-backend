...
function failIfNotAxiosError(err: unknown) {
...
describe("/txs/history", function() {
  ...
  it("should query TXs from the next block if `after.tx` is missing", async() => {
    const result = await axios.post(testableUri, {
    ...
    const txHashes = [
      "ea2b2abb9bf440a8ab89685e359e88399802c002383a21e4d09a7d636a9514d2",
      "065467aa35cd4a53502941b7a9d58ba5f149e45f0f8bd6829a72f8de781ec6c3",
      "e8acd016e530dce91b697ed702f9f8a82a11fc6c43c77074c223378e77152b01",
      "c2e7ad11a93739883102dd9c4ff72143457603678c6c6d9b399128e5977c0829"
    ];

    expect(result.data).to.have.lengthOf(4);

    txHashes.forEach((tx: string, idx: number) => {
      expect(result.data[idx].hash).to.be.equal(tx);
    });
  });
});
