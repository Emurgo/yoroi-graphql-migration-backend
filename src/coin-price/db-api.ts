...
export async function getTickers(db: Pool, fromCurrency: string, timestamps: Array<number>): Promise<Array<Ticker>> {
  const timeValues = await Promise.all(timestamps.map(async (timestamp) => {
    const time = Math.floor(timestamp/1000);
    const result = await db.query({
      text: 'SELECT "time" FROM tickers '+
