...
export const metadataDataFromSmash = async (p: Pool, hash: string): Promise<UtilEither<object>> => {
    ...
    if (metadataHashResp.rows.length === 0) {
        return {kind:"error", errMsg: "metadataDataFromSmash: metadataHashResp empty"};
    }
...
export const poolDelegationHistory = (p: Pool) => async (req: Request, res: Response): Promise<void>=> {
    ...
    if(hashes && hashes.length > addressesRequestLimit)
        throw new Error (`poolRanges must have between 0 and ${addressesRequestLimit} items`);

    const delegationRanges: PoolDelegationRange[] = Object.values(req.body.poolRanges);
    ...
    for (const hash of hashes) {
        ...
        const history: DelegationRangeResponse[] = dbHistory.rows.reduce((result: any[], row: any) => {
